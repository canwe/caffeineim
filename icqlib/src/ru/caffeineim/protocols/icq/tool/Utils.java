...
public class Utils {
    ...
    // Extracts the word from the buffer (buf) at position off using the
    // specified byte ordering (bigEndian)
    ...
    public static int getWord(byte[] buf, int off) {
        return (Utils.getWord(buf, off, true));
    }

    public static String detectUserClient(int dwFP1, int dwFP2, int dwFP3,
            byte[] capabilities, int wVersion) {
        int client = CLI_NONE;
        String szVersion = "";
        ...
        if (capabilities != null) {
            ...
            for (int j = 0; j < capabilities.length / 16; j++) {
                if (Utils.byteArrayEquals(capabilities, j * 16,
                        CAP_AIM_SERVERRELAY, 0, 16)) {
                    caps |= CAPF_AIM_SERVERRELAY_INTERNAL;
                } else if (Utils.byteArrayEquals(capabilities, j * 16,
                        CAP_UTF8, 0, 16)) {
                    ...
                    caps |= CAPF_TRILLIAN;
                        ...
                        CAP_TRILCRYPT, 0, 16)) {
                    caps |= CAPF_TRILCRYPT;
                } else if (Utils.byteArrayEquals(capabilities, j * 16, CAP_SIM,
                        0, 0xC)) {
                    caps |= CAPF_SIM;
                } else if (Utils.byteArrayEquals(capabilities, j * 16,
                        CAP_SIMOLD, 0, 16)) {
                    ...
                    caps |= CAPF_MICQ;
                        ...
                        CAP_ANDRQ, 0, 9)) {
                    caps |= CAPF_ANDRQ;
                } else if (Utils.byteArrayEquals(capabilities, j * 16, CAP_QIP,
                        0, 11)) {
                    caps |= CAPF_QIP;
                } else if (Utils.byteArrayEquals(capabilities, j * 16, CAP_IM2,
                        0, 16)) {
                    caps |= CAPF_IM2;
                } else if (Utils.byteArrayEquals(capabilities, j * 16,
                        CAP_MACICQ, 0, 16)) {
                    ...
                    caps |= CAPF_AIMICON;
                        ...
                        CAP_AIMCHAT, 0, 16)) {
                    caps |= CAPF_AIMCHAT;
                } else if (Utils.byteArrayEquals(capabilities, j * 16, CAP_UIM,
                        0, 16)) {
                    caps |= CAPF_UIM;
                } else if (Utils.byteArrayEquals(capabilities, j * 16,
                        CAP_RAMBLER, 0, 16)) {
                    caps |= CAPF_RAMBLER;
                } else if (Utils.byteArrayEquals(capabilities, j * 16, CAP_ABV,
                        0, 16)) {
                    caps |= CAPF_ABV;
                } else if (Utils.byteArrayEquals(capabilities, j * 16,
                        CAP_NETVIGATOR, 0, 16)) {
                    ...
                    caps |= CAPF_DIRECT;
                        ...
                        CAP_TYPING, 0, 16))
                    caps |= CAPF_TYPING;
                else if (Utils.byteArrayEquals(capabilities, j * 16, CAP_MCHAT,
                        0, 9)) {
                    caps |= CAPF_MCHAT;
                }
            }
            ...
            if (((caps & (CAPF_STR20012 + CAPF_AIM_SERVERRELAY_INTERNAL)) != 0)
                ...
                if ((dwFP1 == 0) && (dwFP2 == 0) && (dwFP3 == 0)
                    ...
                    client = CLI_ICQ2001B; // FP1: 1068985885; FP2:0;
                // FP3:1068986138
    ...
    // Extracts the double from the buffer (buf) at position off using the
    // specified byte ordering (bigEndian)
    ...
    // Extracts the double from the buffer (buf) at position off using big
    // endian byte ordering
    public static long getDWord(byte[] buf, int off) {
        return (Utils.getDWord(buf, off, true));
    }
