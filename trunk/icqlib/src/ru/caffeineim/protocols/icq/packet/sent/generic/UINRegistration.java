 ...
 * <p>Title : JOscarLib (Java Oscar Library)</p>
 * <p><b>This library give access to the Oscar protocol; Part of Object Oriented Instant Messenger Library</b></p>
 * <p> Copyright (c) Fabrice Michellonet 2002, Samolisov Pavel 2007</p>
 * @author Baranov Egor
 */

public class UINRegistration extends Flap
{
    private final static byte[] REGISTRATION_REQUEST_BEGIN = {
                    0x00, 0x00, 0x00, 0x00,
                    0x28, 0x00, 0x03, 0x00,
                    0x00, 0x00, 0x00, 0x00,
                    0x00, 0x00, 0x00, 0x00,
    ...
    private final static byte[] REGISTRATION_REQUEST_END = {
                    0x00, 
                    0x03, 0x46, 0x00, 0x00,
                    0x00, 0x00, 0x00, 0x00,
                    0x11, 0x01
    ...
    public UINRegistration(String password)
    {
        ...
        byte[] regData = new byte[REGISTRATION_REQUEST_BEGIN.length + password.toCharArray().length + REGISTRATION_REQUEST_END.length + 2];
        ...
        {
            regData[i + REGISTRATION_REQUEST_BEGIN.length + 2] = (byte)password.toCharArray()[i];
        }
        
        for (int i = 0; i < REGISTRATION_REQUEST_END.length; i++)
        {
            regData[i + REGISTRATION_REQUEST_BEGIN.length + password.toCharArray().length + 2] = REGISTRATION_REQUEST_END[i];
        }
